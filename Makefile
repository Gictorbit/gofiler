...
build:
	@echo "build server..."
	GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o bin/server github.com/Gictorbit/gofiler/server/cmd
	GOOS=windows GOARCH=amd64 go build -ldflags="-s -w" -o bin/server.exe github.com/Gictorbit/gofiler/server/cmd

	@echo "build client..."
	GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o bin/client github.com/Gictorbit/gofiler/client/cmd
